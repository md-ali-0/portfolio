            ...
            {filteredSkills.map((skill, index) => {
              const colorClasses = {
                ...
                teal: {
                  ...
                  gradient: "from-teal-500/10 via-transparent to-emerald-500/10",
                  iconBg: "from-teal-500 to-emerald-500",
                  particles: "bg-teal-400",
                  glow: "from-teal-400/10 to-emerald-400/10",
                },
                cyan: {
                  ...
                  glow: "from-cyan-400/10 to-teal-400/10",
                },
              }
              ...
              return (
                <motion.div
                  key={skill.name}
                  className={`bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/50 rounded-2xl overflow-hidden group ${colors.border} transition-all duration-500 relative`}
                ...
                >
                  ...
                  {!isMobile && (
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                      {Array.from({ length: 4 }, (_, i) => (
                        <motion.div
                          key={i}
                          className={`absolute w-1.5 h-1.5 ${colors.particles} rounded-full`}
                          style={{
                            left: `${20 + i * 20}%`,
                            top: `${30 + i * 15}%`,
                          }}
                  ...
                  <div className="p-4 md:p-6 text-center relative z-10">
                    ...
                    <div className="flex justify-center mb-4">
                      <div className={`p-3 rounded-xl bg-gradient-to-r ${colors.iconBg} text-black`}>{skill.icon}</div>
                    </div>

                    {/* Title */}
                    <h3 className="text-base md:text-lg font-semibold text-white mb-2">{skill.name}</h3>
                  ...
                  {/* Hover Glow Effect */}
                  <motion.div
                    className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${colors.glow} opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl`}
                    animate={{
                      scale: [1, 1.05, 1],
                    }}
                    transition={{
